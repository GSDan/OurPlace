    ...
    public partial class LoginController : UIViewController
    {
        ...
        private async void OnLoginResult(NSNotification notification)
        {
            try
            {
                ...
                AppLinkUrl appLinkUrl = new AppLinkUrl(notification.UserInfo[new NSString("url")].ToString());

                await safariController.DismissViewControllerAsync(true);
                NSNotificationCenter.DefaultCenter.RemoveObserver(notificationToken);

				if (appLinkUrl.InputQueryParameters.ContainsKey("access_token"))
				{
					accessToken = appLinkUrl.InputQueryParameters["access_token"];
				}
				else { throw new Exception("No access token returned"); }

                if (appLinkUrl.InputQueryParameters.ContainsKey("expires_in"))
                {
                    accessExpiresAt = DateTime.UtcNow.AddSeconds(int.Parse(appLinkUrl.InputQueryParameters["expires_in"]));
                }
                else { throw new Exception("No access token expiry information"); }

                if (appLinkUrl.InputQueryParameters.ContainsKey("refresh_token"))
                {
                    refreshToken = appLinkUrl.InputQueryParameters["refresh_token"];
                }
                else { throw new Exception("No refresh token returned"); }

                if (appLinkUrl.InputQueryParameters.ContainsKey("refresh_token_expires"))
                {
                    refreshExpiresAt = new DateTime(long.Parse(appLinkUrl.InputQueryParameters["refresh_token_expires"]), DateTimeKind.Utc);
                }
                else { throw new Exception("No refresh token expiry information"); }
